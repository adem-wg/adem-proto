...
We assume that you either compiled the respective binaries (see above) or preface them with `go run github.com/adem-wg/adem-proto/cmd/...`.
...
`probe` takes the domain name to query as the first argument.
...
For example, `emblemcheck` could print that `a.example.com` is marked with ADEM although you initially had probed `b.example.com`.

### Emblem Issuers

Emblem issuers can use this prototype to issue emblems that mark their infrastructure as protected.
...
- ...generate a root key that will cryptographically identify your organization, and how to bind this root key to your domain name.
- ...generate emblems.
...
```sh
openssl ecparam -genkey -name secp521r1 -noout -out root_key.pem
```
...
This option takes the respectively provided public or private key's algorithm as a JSON Web Algorithm (see [RFC 7518, Section 3.1](https://datatracker.ietf.org/doc/html/rfc7518#section-3.1)).
...
This allows for holding emblem issuers accountable in case of key misuse.
...
This command will print the corresponding public key to your root key, in the JSON Web Key format.
...
Importantly, observe that the output includes key's key identifier (`kid`), which you require in the following.
For the sake of the example, let's assume your root key's `kid` is `d34db44f`.

Say, your organizations domain name is `example.com`.
To commit to your organization root key, you must request a Web PKI certificate that is valid for both:

- `example.com`, and
- `d34db33f.adem-configuration.example.com`. More generally, for `ROOT_KEY_KID.adem-configuration.example.com`.

You can request such a certificate using Let's Encrypt and the [`certbot`](https://certbot.eff.org/) utility.
On a webserver that can request certificates for `example.com`, you can run:
...
The command will compute information on how to check that the certificate associated with your root key is stored in CT logs.
...
You now successfully configured your organization to issue emblems.
...
Emblem can be generated with the `emblemgen` tool.
To keep domains separated and mitigate damages of leaking private key's, we strongly recommend to use separate keys to sign emblems.
In this section, we will explain how you can mark an asset with an emblem using ADEM.
...
Again, our example will use an ECDSA key using P-521 and SHA-512, but you can use different key types.

```sh
openssl ecparam -genkey -name secp521r1 -noout -out asset_key.pem
```

Next, generate the emblem's "skeleton" file, that is a JSON file that contains all relevant information about the emblem, e.g., the issuer, the marked assets, and the purpose of the emblem.
...
This skeleton file marks the asset as protected (the *purpose* (`prp`) is protected), by reference to its IPv4 address (as an IPv4-mapped IPv6 address) and domain name, and identifies the emblem issuer.
...
Note that some attributes (such as lifetime related attributes) will bet automatically set by `emblemgen`.
...
This command stores the emblem in `emblem.jws`, and signs it as valid for 24h, starting immediately.
If you would like to pick a certain starting point of validity, you can add the `nbf` field to the skeleton file.
...
In order to allow validators to verify that the emblem signing key belongs to your organization, you must issue an *endorsement* for that key that was signed by the root key.
...
The field `end` being `false` encodes that the endorsed key may not endorse further.
The `emb` field includes constraints on emblems that can be issued using the key.
This can help mitigate the consequences of key compromise.
Here, the key is endorsed to only issue emblems for the asset that it is currently being used for.
You can find more information on other fields and constraints of endorsements here: https://www.ietf.org/archive/id/draft-linker-diem-adem-core-00.html#name-endorsements
...
```sh
emblemgen -skey root.pem -alg ES512 -proto endorsement.json -logs root_log.json -pk emblem.pem -lifetime 31536000 > endorsement.jws
```

This command signs an endorsement that is valid for a year of the emblem signing key.
...
Such organizations must also configure a root key as described in the setup section above, but their endorsements will contain an `iss` field different to the `sub` field and must set `end` to `true`.
...
You now signed an emblem and a root endorsement.
You can distribute the emblem and its endorsement via the DNS.
For that, add TXT records to `asset.example.com` that contain the emblem and endorsement respectively, and start with `adem=...`.
