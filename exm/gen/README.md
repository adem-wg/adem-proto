...
This directory provides an example of how to generate tokens.
...
The emblem's claims are defined in `emblem.json` and the endorsement's claims in `endorsement.json`.
Both scripts execute the same command (`emblemgen`).
The command generates endorsements instead of emblems when the `-pk-pem` or `-pk-cbor` argument is provided.

The script `check_trusted.sh` bundles and verifies both CBOR tokens and their COSE keys (as in `exm/vfy`).
